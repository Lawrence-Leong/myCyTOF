...
sigma_alpha <- 2 # Size of batch effects
sigma_epsilon <- 4 # Size of random noise
sigma_biology <- 20 # Size of inherent biology
nbioclust <- 5
...
# Inherent biology
Z <- ruv::replicate.matrix(rep(1:nbioclust, each = ncells/nbioclust))
# Coeficeints of biology
...
bio_ind <- as.matrix(rep(1:nbioclust, each = ncells/nbioclust), nrow = ncells, ncol = 1)
```
...
# Create replciates which respect the biology
...
```{r}
samp <- sample(1:nrow(data), 1000)
tsne_data <- compute_tsne(norm_data, samp)
